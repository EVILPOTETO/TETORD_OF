...
(function(){
  ...
  const O={
    ...
    newOfficeFile(type){
      ...
      this.slidesModule && (this.slidesModule.storagePath=`slides/${Date.now().toString(36)}-${crypto.randomUUID().slice(0,6)}.tetord`); this.slides={items:[{title:'Nueva presentación',body:'',theme:'miku',background:'#dceeff',imagePath:''}],selected:0}; if(this.slidesModule){this.slidesModule.state=this.slides;this.slidesModule.persist().then(()=>this.slidesModule.renderSlides())}else this.save('slides',this.slides);this.touchOfficeMeta('slides');this.open('slides');
    },
