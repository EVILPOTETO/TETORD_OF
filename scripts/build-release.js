...
async function main() {
  ...
  run(npmCommand(), ['exec', '--', 'electron-builder', '--win', 'nsis', 'portable', '--x64']);
  ...
  const forbiddenOutputNames = ['test', 'tests', 'server', 'scripts'];
  const suspicious = entries.filter(name => forbiddenOutputNames.some(part => name.toLowerCase().includes(part)));
